...
router.post("/register", async (req, res, next) => {
  try {
    ...
    if (email) {
      ...
      if (existingUser) {
        // Check if this is a previously deleted account OR user wants to re-register
        if (existingUser.accountDeletedAt || existingUser.accountDeleteRequested) {
          // Account was marked for deletion — force clean up and allow re-registration
