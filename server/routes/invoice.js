...
// ═══════════════════════════════════════════════════════════════
// PARTIAL PAYMENT RECEIPT / PAYMENT INVOICE
// GET /api/invoice/:id/partial
// Available when at least 1 payment recorded (even if not fully paid)
// ═══════════════════════════════════════════════════════════════
router.get("/:id/partial", async (req, res, next) => {
  try {
    ...
    const commPaid = totalPaid * ((booking.commissionPercent || 0) / 100);
    const creatorReceivable = totalPaid - commPaid;
    ...
    const statusBg = isFullyPaid ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)';
    const statusBorder = isFullyPaid ? '#22C55E' : '#F59E0B';
    const statusText = isFullyPaid ? '\u2705 FULLY PAID' : '\uD83D\uDFE1 PARTIALLY PAID';
    const statusBarBg = isFullyPaid ? 'linear-gradient(135deg,#22C55E,#16A34A)' : 'linear-gradient(135deg,#F59E0B,#D97706)';
    const statusBarText = isFullyPaid ? '\u2705 FULLY PAID \u2014 All payments received' : '\u26A0\uFE0F PARTIAL PAYMENT RECEIPT \u2014 Balance pending';
...
.contact-bar{display:flex;gap:16px;font-size:10px;color:#8A8A8A;margin-top:6px;flex-wrap:wrap}
.contact-bar span{color:#CFCFCF}
.inv-card{background:#111;border:2px solid ${statusBorder};border-radius:12px;padding:14px 18px;text-align:center;min-width:220px}
.inv-card .inv-label{font-size:10px;color:#F97316;font-weight:700;letter-spacing:2px;text-transform:uppercase}
.inv-card .inv-no{font-size:14px;font-weight:700;color:#fff;margin-top:4px}
.inv-card .inv-dates{font-size:10px;color:#8A8A8A;margin-top:6px;line-height:1.6}
.status-badge{display:inline-flex;align-items:center;gap:6px;background:${statusBg};border:1px solid ${statusBorder};color:${statusColor};font-size:11px;font-weight:700;padding:6px 14px;border-radius:8px;margin-top:8px}
.sec-title{font-size:14px;font-weight:700;color:#F97316;margin-bottom:12px;display:flex;align-items:center;gap:8px}
.sec-title::before{content:'';width:4px;height:16px;background:#F97316;border-radius:2px}
.grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.party-card{background:#0A0A0A;border:1px solid #2B2B2B;border-radius:12px;padding:16px}
...
.sum-row:last-child{border:none;padding-top:14px;margin-top:6px;border-top:2px solid #F97316}
.sum-row .label{color:#CFCFCF}.sum-row .val{font-weight:700;color:#fff}
.sum-row.green .val{color:#22C55E}.sum-row.orange .val{color:#F97316}.sum-row.red .val{color:#EF4444}.sum-row.yellow .val{color:#F59E0B}
.sum-row.total .label{font-size:15px;font-weight:700;color:#F97316}.sum-row.total .val{font-size:16px;color:#FB923C}
table{width:100%;border-collapse:collapse;font-size:11px;border-radius:10px;overflow:hidden;border:1px solid #2B2B2B}
th{text-align:left;padding:10px 12px;background:#171717;color:#F97316;font-size:9px;text-transform:uppercase;letter-spacing:1px;font-weight:700}
td{padding:10px 12px;border-bottom:1px solid #1a1a1a;color:#CFCFCF}
.status-bar{background:${statusBarBg};border-radius:12px;padding:16px;text-align:center;margin:16px 0;color:#fff;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;gap:8px}
.pending-banner{background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.3);border-radius:12px;padding:14px 18px;margin:16px 0;display:flex;align-items:center;gap:10px}
.pending-banner .pb-icon{font-size:20px}
.pending-banner .pb-text{font-size:11px;color:#F59E0B;line-height:1.6}
.pending-banner .pb-text strong{color:#FCD34D}
.qr-sec{display:flex;align-items:center;gap:16px;background:#111;border:1px solid #2B2B2B;border-radius:12px;padding:16px;margin:16px 0}
.qr-sec img{width:80px;height:80px;border-radius:8px;border:2px solid #F97316;padding:3px;background:#fff}
.qr-text .qr-label{font-size:12px;font-weight:600;color:#CFCFCF}.qr-text .qr-sub{font-size:10px;color:#8A8A8A;margin-top:4px;line-height:1.6}
.footer{text-align:center;padding:20px 0;border-top:1px solid #2B2B2B;margin-top:8px}
...
<div class="page">
<button class="print-btn" onclick="window.print()">Download / Print Payment Receipt</button>
...
<div class="inv-card">
<div class="inv-label">PAYMENT RECEIPT</div>
<div class="inv-no">${receiptNo}</div>
<div class="inv-dates">Date: ${fd(new Date())}<br>Booking Date: ${fd(booking.createdAt)}</div>
...
<div class="ev-item"><div class="ev-label">Location</div><div class="ev-val">${booking.eventLocation || "\u2014"}</div></div>
<div class="ev-item"><div class="ev-label">Booking Date</div><div class="ev-val">${fd(booking.createdAt)}</div></div>
<div class="ev-item"><div class="ev-label">Status</div><div class="ev-val" style="color:${statusColor}">${isFullyPaid ? 'Fully Paid' : 'In Progress'}</div></div>
<div class="ev-item"><div class="ev-label">Receipt #</div><div class="ev-val" style="font-size:10px">${receiptNo}</div></div>
</div>
</div>
...
<div class="sec-title">PAYMENT SUMMARY</div>
<div class="sum-row"><span class="label">Total Project Amount</span><span class="val">${fc(amt)}</span></div>
<div class="sum-row green"><span class="label">Total Paid Till Now</span><span class="val">${fc(totalPaid)}</span></div>
<div class="sum-row ${remaining > 0 ? 'yellow' : ''}"><span class="label">Remaining Balance</span><span class="val">${remaining > 0 ? fc(remaining) + ' (Pending)' : fc(0)}</span></div>
<div class="sum-row red"><span class="label">Platform Commission (${booking.commissionPercent || 0}%)</span><span class="val">${fc(Math.round(commPaid))}</span></div>
<div class="sum-row total"><span class="label">Creator Receivable (on paid amount)</span><span class="val">${fc(Math.round(creatorReceivable))}</span></div>
</div>

<div class="card-dark">
<div class="sec-title">PAYMENT HISTORY (${payments.length} Payment${payments.length > 1 ? 's' : ''})</div>
<table><thead><tr><th>Date</th><th>Amount</th><th>Type</th><th>Recorded By</th><th>Status</th></tr></thead>
<tbody>${payRows}</tbody></table>
</div>
...
${remaining > 0 ? `<div class="pending-banner">
<div class="pb-icon">\u26A0\uFE0F</div>
<div class="pb-text"><strong>Pending Balance: ${fc(remaining)}</strong><br>This is a partial payment receipt. Final invoice will be generated after full payment is completed and the booking is officially closed.</div>
</div>` : ''}

<div class="qr-sec">
<img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&bgcolor=FFFFFF&data=${encodeURIComponent('https://bookmyshot.in | Receipt: ' + receiptNo + ' | Paid: ' + totalPaid + ' | ID: ' + booking._id)}" alt="QR">
<div class="qr-text"><div class="qr-label">Scan to verify this payment</div><div class="qr-sub"><strong>Note:</strong> This is an auto-generated payment receipt from BookMyShot. ${remaining > 0 ? 'The booking is still active with pending balance.' : 'All payments have been received.'} No physical signature required.</div></div>
</div>
